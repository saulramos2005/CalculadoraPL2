...
function TablaIteracionItem({
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-1">
        <div className="flex items-center justify-between w-full sm:w-auto gap-2">
          ...
          <div className="py-3 px-2 bg-cyan-50 border border-cyan-200 dark:bg-cyan-900/20 dark:border-cyan-800 rounded flex flex-row justify-between gap-2 text-sm">
            <div className="flex items-center gap-1">
              <ChevronRight className="size-4 text-cyan-600 dark:text-cyan-400" />
            ...
            {tabla.columnaPivote !== undefined &&
              tabla.filaPivote !== undefined ? (
                <div className="text-cyan-900 dark:text-cyan-100 flex  gap-x-1 gap-y-1 sm:gap-x-2 text-center">
                  <span>
                    Entra:{" "}
                    <strong className="text-emerald-700 dark:text-emerald-400">
                      {tabla.encabezados[tabla.columnaPivote]}
                    </strong>
                  </span>
                  <span className="inline text-cyan-300 dark:text-cyan-800">
                    |
                  </span>
                  <span>
                    Sale:{" "}
                    <strong className="text-rose-700 dark:text-rose-400">
                      {tabla.basicas[tabla.filaPivote]}
                    </strong>
                  </span>
                  <span className="inline text-cyan-300 dark:text-cyan-800">
                    |
      ...
      {tabla.es_optima && (
        <div className="mt-2 px-2 py-1.5 bg-emerald-50 border border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800 rounded text-sm text-emerald-900 dark:text-emerald-300">
          Todos los coeficientes en la fila Z son ≥ 0. Se alcanzó la solución
          óptima.
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="h-96 w-full rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900" />
      )}
